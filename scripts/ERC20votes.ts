...
async function main() {
  ...
  console.log(
    `Tokens minted for ${account1.address} at block ${mintTxReceipt1.blockNumber}`
  );
  ...
  const mintTx2Receipt2 = await mintTx2.wait();
  console.log(
    `Tokens minted for ${account2.address} at block ${mintTx2Receipt2.blockNumber}`
  );
  ...
  let votePowerAccount2 = await contract.getVotes(account2.address);
  console.log(
    `Account 2 has a vote power of ${ethers.utils.formatEther(
      votePowerAccount2
    )} units`
  );

  // Self delegate
  const delegateTx1 = await contract
    .connect(account1)
  ...
  console.log(
    `Tokens delegated from ${account1.address} for ${
      account1.address
    } at block ${delegateTxReceipt1.blockNumber}, for a cost of ${
      delegateTxReceipt1.gasUsed
    } gas units, totaling a tx cost of ${delegateTxReceipt1.gasUsed.mul(
      delegateTxReceipt1.effectiveGasPrice
    )} Wei (${ethers.utils.formatEther(
      delegateTxReceipt1.gasUsed.mul(delegateTxReceipt1.effectiveGasPrice)
    )} ETH)`
  );
  ...
  const delegateTxReceipt2 = await delegateTx1.wait();
  console.log(
    ...
    } gas units, totaling a tx cost of ${delegateTxReceipt2.gasUsed.mul(
  ...
  console.log(
    `Tokens transferred from ${account1.address} for ${account2.address} at block ${transferTxReceipt.blockNumber}`
  );
  ...
  console.log(
    `Account 2 had a vote power of ${ethers.utils.formatEther(
      votePowerAccount1
  ...
  const ballotContract = await ballotContractFactory.deploy(
    ...
    contract.address,
    delegateTxReceipt2.blockNumber
  );
  ...
  const voteTx1 = await ballotContract
    .connect(account1)
    .vote(0, ethers.utils.parseEther("2"));
  ...
  const voteTx3 = await ballotContract
    .connect(account2)
  ...
  let votingPowerRemainingAcct1 = await ballotContract
    .connect(account1)
    .votingPower(account1.address);
  console.log(
    `account1 has a remaining voting power of ${votingPowerRemainingAcct1}`
  );

  // checks voting power spent for account 2
  let votingPowerRemainingAcct2 = await ballotContract
    .connect(account2)
    .votingPower(account2.address);
  console.log(
    `account1 has a remaining voting power of ${votingPowerRemainingAcct2}`
  );
